...
on_keymap_binding_convert_central_state_dependent_params(struct zmk_behavior_binding_event *event) {
    ...
    case RGB_TOG_CMD: {
        ...
        event->param1 = state ? RGB_OFF_CMD : RGB_ON_CMD;
        break;
    }
    case RGB_BRI_CMD: {
        struct zmk_led_hsb color = zmk_rgb_underglow_calc_brt(1);

        event->param1 = RGB_COLOR_HSB_CMD;
        event->param2 = RGB_COLOR_HSB_VAL(color.h, color.s, color.b);
        break;
    }
    case RGB_BRD_CMD: {
        struct zmk_led_hsb color = zmk_rgb_underglow_calc_brt(-1);
    ...
    case RGB_EFR_CMD: {
        event->param1 = RGB_EFS_CMD;
        event->param2 = zmk_rgb_underglow_calc_effect(-1);
        break;
    }
    case RGB_EFF_CMD: {
        event->param1 = RGB_EFS_CMD;
    ...
    LOG_DBG("RGB relative convert to absolute (%d/%d)", event->param1, event->param2);
...
static int on_keymap_binding_pressed(struct zmk_behavior_binding_event *event) {
    ...
    case RGB_SPD_CMD:
        return zmk_rgb_underglow_change_spd(-1);
    case RGB_EFS_CMD:
        return zmk_rgb_underglow_select_effect(event->param2);
    case RGB_EFF_CMD:
        return zmk_rgb_underglow_cycle_effect(1);
    case RGB_EFR_CMD:
        return zmk_rgb_underglow_cycle_effect(-1);
    case RGB_COLOR_HSB_CMD:
        return zmk_rgb_underglow_set_hsb((struct zmk_led_hsb){.h = (event->param2 >> 16) & 0xFFFF,
                                                              .s = (event->param2 >> 8) & 0xFF,
                                                              .b = event->param2 & 0xFF});
    }
