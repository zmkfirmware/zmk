...
struct behavior_hold_tap_config {
    ...
    int32_t prior_idle_ignore_key_positions_len;
    int32_t *prior_idle_ignore_key_positions;
};
...
// keycode listener to call update_last_non_ignored_timestamps with the correct
...
static bool is_prior_idle_ignore_key_position(const struct behavior_hold_tap_config *config,
                                              int32_t position) {
    for (int i = 0; i < config->prior_idle_ignore_key_positions_len; i++) {
        if (config->prior_idle_ignore_key_positions[i] == position) {
            return true;
        }
    }
    return false;
...
// position listener can update per-instance last_non_ignored_key_timestamp.
...
static void update_last_non_ignored_timestamps(int32_t position, int64_t timestamp) {
    for (int i = 0; i < ht_num_devices; i++) {
        ...
        if (cfg->prior_idle_ignore_key_positions_len > 0 &&
            !is_prior_idle_ignore_key_position(cfg, position)) {
            data->last_non_ignored_key_timestamp = timestamp;
        }
    }
}

static bool is_quick_tap(struct active_hold_tap *hold_tap) {
    if (hold_tap->config->prior_idle_ignore_key_positions_len > 0) {
        if ((hold_tap->data->last_non_ignored_key_timestamp +
...
static int keycode_state_changed_listener(const zmk_event_t *eh) {
    ...
    if (ev->state && !is_mod(ev->usage_page, ev->keycode)) {
        ...
        update_last_non_ignored_timestamps(last_keydown_position, ev->timestamp);
    }
...
static int behavior_hold_tap_init(const struct device *dev) {
    ...
    struct behavior_hold_tap_data *data = dev->data;
    data->last_non_ignored_key_timestamp = INT32_MIN;
...
#define KP_INST(n)                                                                                 \
    COND_CODE_1(DT_INST_NODE_HAS_PROP(n, hold_trigger_key_positions),                              \
                ...
                ())                                                                                \
    COND_CODE_1(DT_INST_NODE_HAS_PROP(n, prior_idle_ignore_key_positions),                         \
                (static const int32_t prior_idle_ignore_key_positions_##n[] =                      \
                     DT_INST_PROP(n, prior_idle_ignore_key_positions);),                           \
                ())                                                                                \
    static const struct behavior_hold_tap_config behavior_hold_tap_config_##n = {                  \
        ...
        .prior_idle_ignore_key_positions =                                                         \
            COND_CODE_1(DT_INST_NODE_HAS_PROP(n, prior_idle_ignore_key_positions),                 \
                        (prior_idle_ignore_key_positions_##n), (NULL)),                            \
        .prior_idle_ignore_key_positions_len =                                                     \
            DT_INST_PROP_LEN(n, prior_idle_ignore_key_positions),                                  \
    };                                                                                             \
