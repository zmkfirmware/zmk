...
#define COND_POLL_OR_INTERRUPTS(pollcode, intcode)                                                 \
    COND_CODE_1(CONFIG_ZMK_INPUT_DIRECT_POLLING, pollcode, intcode)
...
#define INPUT_GPIO_DIRECT_INPUT_CFG_INIT(idx, inst_idx)                                            \
    INPUT_GPIO_GET_BY_IDX(DT_DRV_INST(inst_idx), input_gpios, idx)
#define INPUT_KEY_DIRECT_INPUT_CFG_INIT(idx, inst_idx)                                             \
    INPUT_GPIO_GET_BY_IDX(DT_INST_PROP_BY_IDX(inst_idx, input_keys, idx), gpios, 0)
...
static gpio_flags_t input_gpio_get_extra_flags(const struct gpio_dt_spec *gpio, bool active) {
    if (!active) {
...
static int input_inputs_set_flags(const struct input_gpio_list *inputs,
    ...
    for (int i = 0; i < inputs->len; i++) {
        ...
        const gpio_flags_t extra_flags =
            GPIO_INPUT | input_gpio_get_extra_flags(&inputs->gpios[i].spec, active);
...
static int input_direct_read(const struct device *dev) {
    ...
    for (int i = 0; i < data->inputs.len; i++) {
        ...
        const int active = input_gpio_pin_get(gpio, &state);
        if (active < 0) {
    ...
    for (int i = 0; i < data->inputs.len; i++) {
        ...
        if (zmk_debounce_get_changed(deb_state)) {
            ...
            if (config->toggle_mode && pressed) {
                input_inputs_set_flags(&data->inputs, &gpio->spec);
            }
        }
...
static int input_direct_init_input_inst(const struct device *dev, const struct gpio_dt_spec *gpio,
    ...
    int err = gpio_pin_configure_dt(
        gpio, GPIO_INPUT | (toggle_mode ? input_gpio_get_extra_flags(gpio, false) : 0));
    if (err) {
...
#if USE_INTERRUPTS
    struct input_direct_data *data = dev->data;
    struct input_direct_irq_callback *irq = &data->irqs[index];

    irq->dev = dev;
    gpio_init_callback(&irq->callback, input_direct_irq_callback_handler, BIT(gpio->pin));
...
static int input_direct_init_inputs(const struct device *dev) {
    ...
    for (int i = 0; i < data->inputs.len; i++) {
        const struct gpio_dt_spec *gpio = &data->inputs.gpios[i].spec;
        int err = input_direct_init_input_inst(dev, gpio, i, config->toggle_mode);
        if (err) {
            return err;
        }
    }
...
#define INPUT_DIRECT_INIT(n)                                                                       \
    ...
    BUILD_ASSERT(INST_DEBOUNCE_RELEASE_MS(n) <= DEBOUNCE_COUNTER_MAX,                              \
                 "ZMK_INPUT_DEBOUNCE_RELEASE_MS or debounce-release-ms is too large");             \
                                                                                                   \
    static struct input_gpio input_direct_inputs_##n[] = {                                         \
        COND_CODE_1(DT_INST_NODE_HAS_PROP(n, input_gpios),                                         \
                    (LISTIFY(INST_INPUTS_LEN(n), INPUT_GPIO_DIRECT_INPUT_CFG_INIT, (, ), n)),      \
                    (LISTIFY(INST_INPUTS_LEN(n), INPUT_KEY_DIRECT_INPUT_CFG_INIT, (, ), n)))};     \
                                                                                                   \
    ...
    static struct input_direct_data input_direct_data_##n = {                                      \
        .inputs = INPUT_GPIO_LIST(input_direct_inputs_##n),                                        \
