...
/** GPIO_DT_SPEC_GET_BY_IDX(), but for a struct input_gpio. */
#define INPUT_GPIO_GET_BY_IDX(node_id, prop, idx)                                                  \
    ((struct input_gpio){.spec = GPIO_DT_SPEC_GET_BY_IDX(node_id, prop, idx), .index = idx})

struct input_gpio_list {
    struct input_gpio *gpios;
    size_t len;
};
...
struct input_gpio_port_state {
    const struct device *port;
    gpio_port_value_t value;
};

/**
 * Sorts a GPIO list by port so it can be used with input_gpio_pin_get().
 */
 ...
 * pins in a list which is sorted by input_gpio_list_sort_by_port(), it only
