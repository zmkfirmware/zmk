...
Next we will define the roles for the unibody keyboard. The keyboard will be set as the peripheral and the dongle will be set as the central.

There is also an option to still use the keyboard as the central, and the dongle will not be used.
...
if SHIELD_MY_BOARD
# Setting this to y will make the unibody keyboard the central and the dongle is not used
config ZMK_SPLIT_ROLE_CENTRAL
    default n
