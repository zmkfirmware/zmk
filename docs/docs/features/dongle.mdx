...
ZMK split and unibody keyboards use BLE for wireless communication. Split keyboards consist of two halves, each with its own microcontroller and battery. The left half usually serves as the central role, which drains its battery faster.
A dongle can act as a new central microcontroller, communicating with both halves and connecting to the host via USB. This allows the central half to draw power from the host, extending battery life.
For unibody keyboards, the dongle also helps by setting the unibody keyboard as peripheral, optimizing battery usage.

The main benefit of using a dongle is extended and balanced battery life, in some cases extending from a few weeks to several months.
...
:::info
For more information on how to set up a unibody or split keyboard, refer to [New Keyboard Shield](../development/new-shield.mdx) under Guides.
After that is done, you can follow the steps below to add a dongle to the setup.
:::
...
- For unibody keyboards, the dongle sets the unibody keyboard as peripheral, improving battery life.
- For split keyboards, adding a dongle enables both keyboard halves to act as peripherals, meaning equal improved battery life for both halves.
...
:::note
More information about [User Config Folder](../config/index.md#user-config-folder)
:::
