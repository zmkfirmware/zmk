...
```bash
bash -c "$(curl -fsSL https://v0-3-branch.zmk.dev/setup.sh)"
```
...
```bash
bash -c "$(wget https://v0-3-branch.zmk.dev/setup.sh -O -)" '' --wget
```
...
```powershell
powershell -Command "iex ((New-Object System.Net.WebClient).DownloadString('https://v0-3-branch.zmk.dev/setup.ps1'))"
```
