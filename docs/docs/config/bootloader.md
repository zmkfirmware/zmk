...
These are general settings that control the various bootloader intergration feateres of ZMK.
...
Some target SoCs may use the bootmode magic value mapper for [bootloader intergration](docs/development/hardware-integration/bootloader/index.mdx). When doing so, the following configurations are used:
