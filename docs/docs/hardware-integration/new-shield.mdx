...
```kconfig title="boards/shields/<keyboard_name>/Kconfig.shield"
...
ZMK supports split keyboards with more than two parts, repeat the above for each part of your keyboard.

<details>
<summary>Split keyboards with more than two parts</summary>

For split keyboards with more than two parts, you can define additional flags for each part of your keyboard. Each keyboard part should have a unique shield name. For example:
...
# No whitespace after the comma or in your part name!
config SHIELD_MY_KEYBOARD_NUMPAD
    def_bool $(shields_list_contains,my_keyboard_numpad)
```
...
```kconfig title="boards/shields/<keyboard_name>/Kconfig.defconfig"
...
For split keyboards with more than two parts, adjust the number of peripheral parts and BLE connection settings:
...
# If not set, the default is 1 peripheral part
# Set to the number of peripheral parts in your split keyboard
...
# List all Kconfig flags for each part of your split keyboard here, e.g.:
if SHIELD_MY_KEYBOARD_LEFT || SHIELD_MY_KEYBOARD_RIGHT || SHIELD_MY_KEYBOARD_NUMPAD
