...
Most behaviors will also define a pair of functions named `on_<behavior_name>_binding_pressed` and `on_<behavior_name>_binding_released` that accept `(struct zmk_behavior_binding *binding, struct zmk_behavior_binding_event event)` as parameters.
These will be assigned to the API, to be called when the behavior is invoked with a press or a release respectively.
...
- `drivers/behavior.h`: ZMK Behavior Functions (e.g. [`locality`](#api-structure))
...
- `zmk/events/zmk_behavior_binding_event.h`: ZMK Behavior Structs (e.g. parameters, position and timestamp of events)
  - `struct`s:
    ...
    - `zmk_behavior_binding_event`: Contains a behavior binding, layer, position, source, event type (`PRESS`, `RELEASE`, or `SENSOR`), and timestamp data
...
Including `zmk/event_manager.h` is required to listen to or raise the following events:

- `zmk/events/zmk_behavior_binding_event.h`: As described previously. Raise this event and return the result if your behavior should invoke another behavior. If your behavior wishes to invoke multiple behaviors, you should use `zmk/behavior_queue.h` instead.
...
- `.binding_pressed`: Used for behaviors that invoke an action on its keybind press. Set `.binding_pressed` equal to the function typically named [`on_<behavior_name>_binding_pressed`](#creating-the-driver).
- `.binding_released`: Same as above, except for activating on keybind release events. Set `.binding_released` equal to the function typically named [`on_<behavior_name>_binding_released`](#creating-the-driver).
