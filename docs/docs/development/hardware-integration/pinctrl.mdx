...
On designs using wired split on nRF52840, the interrupt priority of the UART may need to be lowered to ensure that the interrupts used to handle timing sensitive BT interactions can repond when needed.

This can be accomplished by overwriding the `interrupts` property and passing a larger number/lower priority for the second value, e.g.:
...
The first number is the interrupt number, which needs to be unchanged. You can find the correct interrupt number for ths eelected UART by inspecting the Zephyr [`nRF52840.dtsi`](https://github.com/zmkfirmware/zephyr/blob/v3.5.0%2Bzmk-fixes/dts/arm/nordic/nrf52840.dtsi#L101) file.
