...
:::tip
Some boards, such as the Seeed Studio Xiao, already have some of their pins configured for particular communication protocols. If this is the case, then you can use the pre-configured bus rather than configuring one manually (you merely need to identify the correct label for the bus node).
:::
...
:::danger
I have not tested this to verify. I _think_ it is correct. If it is not, then we should recommend forking the source, editing it, and using the fork to build while waiting for a PR to be approved.
:::

If the shield that you are using is found in-tree of ZMK or within an external module, and _does not_ contain the overlay for the board that you are using, then you can still define the overlay in your local zmk-config. For example, if you wanted to use the "mikoto_520" with the "sofle" shield, then your ZMK config would contain the following structure (with the folders and files other than the `boards` folder populated as usual):
...
You would then need to make sure that the bus node defined in `mikoto_520.overlay` has the same label as in the other board overlay files of the "sofle".

:::note
It is _strongly_ recommended that in addition to doing the above as a "temporary" fix, you submit a pull request to ZMK or the external module adding the additional board overlay.
:::
...
- The nRF52840 has its pins marked as either low frequency or high frequency.
  - Low frequency pins are limited to a frequency of 10KHz, making them unusable for I2C, UART, and (most of) SPI.
...
2. Defining a driver node and assigning pin groups to it

It is standard to do this in two separate files. Create a file called `<my-board>-pinctrl.dtsi`, which will be used for the first part. This file will later be imported into your board's `.dts`. To avoid namespace collisions, if you are configuring pin control for a [shield](#boards-shields-and-modules), then you should write the contents of both parts into the `.overlay` file directly.
...
:::info
Some MCUs/SoCs may have their headers located in a Zephyr HAL module. See for example the [NXP HAL module](https://github.com/zephyrproject-rtos/hal_nxp/tree/master/dts/nxp/nxp_imx/rt).
:::
...
&pinctrl {
    ...
    spi0_default: spi0_default {
        ...
        group1 {
            /* main role: configure P0.1 as SPI clock, P0.2 as SPI MOSI, P0.3 as SPI MISO */
...
You'll want to identify the correct node for you to be changing. The nRF52840 has nodes defined in `dts/arm/nordic/nrf52840.dtsi`, while the rp2040 has nodes defined in `dts/arm/rpi_pico/rp2040.dtsi`. **Make sure the node that you're using is not already in use elsewhere.**
...
Additional information on configuring specific SPI buses can be found in other pages of the ZMK documentation, or in the Zephyr documentation.
