...
The most popular form of homerow mods is known as "timeless homerow mods", configured to minimize the dependency on timing.
Timeless homerow mods define both a "left hand" and a "right hand" behavior.
...
For homerow mods, it is recommended to use this property with `hold-trigger-on-release` so that modifiers on the same hand can be combined.
