...
A hold-tap behavior has a 'hold' behavior and a 'tap' behavior.
When the key is held, then it will output the hold behavior, and when it is tapped it will output the tap behavior.

Various configuration options exist to allow fine-tuning of whether a particular keypress will resolve in a hold or in a tap.
Examples of such configuration options are how to treat "interrupts" when one key is pressed while a hold-tap has not been released yet, or how long a key should be held before being treated as a hold vs a tap.
...
This corresponds to the [hold-preferred flavor](#interrupt-flavors).
...
Note that you can also [define a custom hold-tap](#custom-hold-tap-examples) instead of adjusting the default behavior.

## Layer-Tap

The "layer-tap" behavior works identically to the mod-tap behavior, but instead of outputting one of two keys, it enables a specified layer as the "hold".
...
The below examples are intended for you to copypaste as desired, and then tune with the configuration options.
...
Name and label them something else, e.g. `mt2: mod_tap_2`, to avoid this.
You'll then be able to use your new behavior with `&mt2`.

</TabItem>
<TabItem value="homerow_mods">

The most popular form of homerow mods is known as "timeless homerow mods", configured to minimise the dependency on timing.
Timeless homerow mods use both a "left hand" and a "right hand" behavior.
...
\ {
    behaviors {
        hml: home_row_mod_left {
            ...
            quick-tap-ms = <175>;
            bindings = <&kp>, <&kp>;
            hold-trigger-key-positions = < ... >; // List of keys on the left hand side of the keyboard
...
If you wish to use this configuration of homerow mods, you will need to define the corresponding left and right behaviors. Below is a brief overview of the options set.
...
- [`require-prior-idle-ms`](#require-prior-idle-ms) is set to 125ms to resolve to taps immediately when typing at speed, which all but completely eliminates any typing delay.
...
- [`quick-tap-ms`](#quick-tap-ms) is set to 175ms, for the event where a key may wish to be held down.

Depending on your typing behavior, you may wish to adjust this configuration somewhat. For example, adjusting the specific timings used, or switching the flavor to `tap-unless-interrupted`.
...
This value defines how long a key must be pressed to trigger Hold behavior. The default is 200ms.
...
The default is called "hold-preferred". It works very well for a ctrl/escape key, but is not very well suited for home-row mods or layer-taps.

ZMK's hold-tap behaviors can be configured with the follow flavors:
...
This is useful for things like a backspace, where a quick tap+hold holds backspace pressed.
Set this to a negative value to disable. The default is -1 (disabled).
...
This effectively disables the hold-tap when typing quickly, which can be quite useful for homerow mods.
It can also have the effect of removing the input delay when typing quickly.
